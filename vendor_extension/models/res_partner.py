...
class ResPartner(models.Model):
    ...
    def _default_vendor_status(self):
        user = self.env['res.users'].search([('active', '=', True)])
        for rec in self.env['vendor.status'].search([]):
            if len(rec.change_user_ids) == len(user):
                return rec

    vendor_status_id = fields.Many2one('vendor.status', default=_default_vendor_status, required=True)
    ...
    def write(self, values):
        ...
        if 'vendor_status_id' in values:
            new_status = self.env['vendor.status'].browse(
                values['vendor_status_id'])
